'use strict';
app.controller('AttendancesController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"attendances/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'attendances/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/attendances-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.submit_frm_edit = function(edit_fields)
    {
        if(form_valid('#frmEdit',"help-block")){
            ...
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"attendances/edit", edit_fields, '/attendances');
    ...
    $rootScope.data_active_inactive = function(item){
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"attendances/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'attendances/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"attendances","d_date"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"attendances/attendances-delete",id);
...
'use strict';
app.controller('AddAttendancesController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    ...
    /*$scope.enable_button_student = function(){
        $timeout(function ()
        {   
           $scope.checkAll = false;
           $scope.check_count = 0;
           angular.forEach($scope.selected_student, function(index,val){
                if(index){
                    if(val == '0' )
                    {
                        $scope.selected_student[0] = false;
                    }
                    else
                    {
                        $scope.student_id.push(val);
                        $scope.check_count++;
                    }
                }
           });
    ...
    /*$scope.check_all_student = function(checkAll,parent) { 
      $timeout(function(){
            $scope.students_list = $filter('filter')($rootScope.students_list, {i_batch_id:$rootScope.add_fields.i_batch_id});
            angular.forEach($scope.students_list, function(data){
                $scope.selected_student[data.id] = checkAll; 
            }); 
            if(parent){
                $scope.selected_student[0] = false;
            }
    ...
    $scope.selectStudent = function(student_id,active){
        ...
        $scope.students_list = $filter('filter')($rootScope.students_list, {i_batch_id:$rootScope.add_fields.i_batch_id});
        if($scope.check_count == $scope.students_list.length)
        {
            angular.element(".group-checkable").parent("span").addClass("checked");
            $scope.selected_student[0] = true;
        }
        else
        {
            angular.element(".group-checkable").parent("span").removeClass("checked");
    ...
    $scope.check_all_student = function() {
        $timeout(function(){
            $scope.student_id = []; 
            $scope.students_list = $filter('filter')($rootScope.students_list, {i_batch_id:$rootScope.add_fields.i_batch_id});
            if ($scope.selectedAll) {
                $scope.selectedAll = true;
                angular.element(".student_chk").parent("span").addClass("checked");
                ...
                angular.element(".student_chk").parent("span").removeClass("checked");
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        if(form_valid('#frmAdd',"help-block")){
            ...
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"attendances/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"attendances/add", add_fields, '/attendances');
            }
