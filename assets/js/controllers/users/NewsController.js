'use strict';
app.controller('NewsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"news/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'news/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/news-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.submit_frm_edit = function(edit_fields)
    {
        if(form_valid('#frmEdit',"help-block")){
            ...
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"news/edit", edit_fields, '/news');
    ...
    $rootScope.data_active_inactive = function(item){
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"news/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'news/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"news","i_batch_id"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"news/news-delete",id);
...
'use strict';
app.controller('AddNewsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        if(form_valid('#frmAdd',"help-block")){
            ...
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"news/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"news/add", add_fields, '/news');
            }
