'use strict';
app.controller('StudentsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"students/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'students/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/students-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.submit_frm_edit = function(edit_fields)
    {
        if(form_valid('#frmEdit',"help-block")){
            ...
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"students/edit", edit_fields, '/students');
    ...
    $rootScope.data_active_inactive = function(item){
        ...
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"students/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'students/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"students","v_name"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"students/students-delete",id);
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_USER_URL+"students","v_first_name");
        $rootScope.get_all_common_list($scope.sectionName);
...
'use strict';
app.controller('AddStudentsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    $rootScope.batches_list = {};
    $rootScope.schools_list = {};
    $scope.sectionName = 'students';
    ...
    responsePromise.success(function(data, status1, headers, config) 
    {
        ...
        $rootScope.schools_list = data.schools_list;
    });
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        if(form_valid('#frmAdd',"help-block")){
            ...
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"students/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"students/add", add_fields, '/students');
            }
