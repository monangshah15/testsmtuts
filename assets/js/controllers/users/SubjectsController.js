'use strict';
app.controller('SubjectsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"subjects/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'subjects/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/subjects-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.submit_frm_edit = function(edit_fields){
        ...
        if(form_valid('#frmEdit',"help-block")){
            ...
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"subjects/edit", edit_fields, '/subjects');
    ...
    $rootScope.data_active_inactive = function(item){
        ...
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"subjects/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'bacthes/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"subjects","v_name"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"subjects/subjects-delete",id);
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_USER_URL+"subjects","v_subject_name");
...
'use strict';
app.controller('AddSubjectsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    $rootScope.user_permission = '';
