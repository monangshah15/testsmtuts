'use strict';
app.controller('SchoolsController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"schools/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'schools/export/'+$scope.someModelSerialized;
    }
    ...
    $scope.fileNameSplit = function(string, nb) {
        $scope.array = string.substring(string.indexOf('_')+nb)
        return $scope.result = $scope.array;
    }
    
    angular.element(document.querySelector('#job_file_upload')).on('change',function(evt){
        var file=evt.currentTarget.files[0];
        console.log(evt);
        /*angular.element("#uploaded_file_name").html(file.name);*/
        //$scope.add_fields.upload_img_name = file.name;
        angular.element("#job_uploaded_file_name").html(file.name);
        $scope.edit_fields.job_file_name = file.name;
        var file=evt.currentTarget.files[0];
        ...
        reader.onload = function (evt) {
            ...
            $scope.$apply(function($scope) {
                    if(file_size < GLOBAL.MAX_UPLOAD_SIZE*1000) {
                        if(angular.element(".download-link").length > 0){
                            angular.element(".download-link").hide();
                        } 
                        $scope.validImage = 1;
                        $rootScope.enableEdit = true;
                        $scope.imageDataURI = evt.target.result;      
                        $scope.edit_fields.v_doc_edit = evt.target.result;
                        angular.element("#v_doc_edit").val(evt.target.result);
                    } else {
                        $scope.validImage = 0;
                        bootbox.alert('Your file is larger than the maximum allowed limit of 5MB.');
                    }
            });
        };
        $timeout(function() {
    ...
    /*angular.element(document.querySelector('#job_sign_img_upload')).on('change',function(evt){
        var file=evt.currentTarget.files[0];
        console.log(evt);
        $scope.edit_fields.v_sign_img_name = file.name;
        var file=evt.currentTarget.files[0];
        var reader = new FileReader();
        reader.onload = function (evt) {
            var w = this.width, h = this.height, t = file.type, n = file.name, file_size = ~~(file.size/1024); // ext only: // file.type.split('/')[1],
            ...
            $scope.$apply(function($scope) {
                if (evt.target.result.toLowerCase().indexOf("data:image/png;base64") >= 0 || evt.target.result.toLowerCase().indexOf("data:image/jpg;base64") >= 0 || evt.target.result.toLowerCase().indexOf("data:image/gif;base64") >= 0 || evt.target.result.toLowerCase().indexOf("data:image/jpeg;base64") >= 0) {
                    if(file_size < GLOBAL.MAX_UPLOAD_SIZE*1000) {
                        $scope.validImage = 1;
                        $rootScope.enableImgEdit = true;
                        $scope.imageDataURI = evt.target.result;      
                        $scope.edit_fields.v_sign_img_edit = evt.target.result;
                    } else {
                        $scope.validImage = 0;
                        bootbox.alert('Your file is larger than the maximum allowed limit of 5MB.');
                    }
                    ...
                    $scope.validImage = 0;  
                    bootbox.alert('Please upload image file only.');                                               
                }
            });
        };
        $timeout(function() {
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        if(form_valid('#frmAdd',"help-block")){
            add_fields.v_doc = $("#v_doc").val();
            //Call submit data services
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"schools/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"schools/add", add_fields, '/schools');
            }
        } else {           
        return false;
        }                
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/schools-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.frm_edit_open = function(item)
    {
        angular.element('.alert').hide();
        $rootScope.edit_fields = item;
        $rootScope.edit_fields.d_date = $filter('date')(item.d_date, 'dd-MM-yyyy');
        $rootScope.edit_fields.d_coc_date = $filter('date')(item.d_coc_date, 'dd-MM-yyyy');
    ...
    $rootScope.submit_frm_edit = function(edit_fields){
        if(form_valid('#frmEdit',"help-block")){
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"schools/edit", edit_fields, '/schools');
    ...
    $rootScope.data_active_inactive = function(item){
        ...
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"schools/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'schools/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            //PaginationService.loading(GLOBAL.API_USER_URL+"schools","v_title"); 
                            angular.forEach($rootScope.ids , function(index)
                            {
                                angular.element('#tablerow'+index).fadeOut('2000');
                            })
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"schools/schools-delete",id);
    ...
    $scope.$on('$viewContentLoaded', function() {
        //loadSingleDatePicker('frmSearchForm');
        $http.defaults.headers.post['Content-Type'] = $headerData;
        $rootScope.fields_reset = 0;
        PaginationService.loading(GLOBAL.API_USER_URL+"schools","v_title");
        angular.element(document.querySelector('#v_doc')).on('change',function(evt) 
        {     
            var file=evt.currentTarget.files[0];
            angular.element("#uploaded_file_name").html(file.name);
        });
