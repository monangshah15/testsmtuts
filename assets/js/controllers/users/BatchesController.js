'use strict';
app.controller('BatchesController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"batches/data/"+$stateParams.id);
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'batches/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
        ...
        if(form_valid('#frmAdd',"help-block")){
            ...
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"batches/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"batches/add", add_fields, '/batches');
            }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/batches-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.frm_edit_open = function(item)
    {
        ...
        /*var responsePromise = $http.post(GLOBAL.API_USER_URL+"batches/check-auth-permission");
        responsePromise.success(function(data, status1, headers, config) 
        {
    ...
    $rootScope.submit_frm_edit = function(edit_fields){
        ...
        if(form_valid('#frmEdit',"help-block")){
            ...
            //Call submit data services            
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"batches/edit", edit_fields, '/batches');
    ...
    $rootScope.data_active_inactive = function(item){
        ...
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"batches/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'batches/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"users","v_name"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"batches/batches-delete",id);
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_USER_URL+"batches","v_batch_title");
...
'use strict';
app.controller('AddBatchesController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    //$rootScope.user_list = {};
    /*var responsePromise = $http.post(GLOBAL.API_USER_URL+"get-all-lists");
    responsePromise.success(function(data, status1, headers, config) 
    {
        if(data == "" || data == undefined)
        {
        ...
        $rootScope.add_fields.user_list = data.user_list;
    });*/
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
