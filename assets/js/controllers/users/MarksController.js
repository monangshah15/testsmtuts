'use strict';
app.controller('MarksController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    ...
    $rootScope.max_allow_marks = '';
    $scope.sectionName = 'marks';
    ...
    if($stateParams.id != "" && $stateParams.id != undefined)
    {
        if(mySharedService.edit_fields != undefined) {
            ...
            var responsePromise = $http.post(GLOBAL.API_USER_URL+"marks/data/"+$stateParams.id);
            responsePromise.success(function(data, status1, headers, config) 
            {
                if(data.items != "" && data.items != undefined)
                {
                    $timeout(function()
                    {
                        ...
                        $rootScope.max_allow_marks  = $rootScope.items.exams.i_total_marks;
    ...
    $rootScope.export_to_excel = function(fields){
        ...
        window.location.href = GLOBAL.API_USER_URL+'marks/export/'+$scope.someModelSerialized;
    }
    ...
    $rootScope.loadEditData = function(item){
        ...
        $state.go('/marks-edit',{id: item.id , edit_fields: item});
    }
    ...
    $rootScope.frm_edit_open = function(item)
    {
        ...
        if($rootScope.edit_fields.i_total_marks !== undefined){
            $rootScope.max_allow_marks  = $rootScope.edit_fields.i_total_marks;    
        } else {
            $rootScope.max_allow_marks  = $rootScope.items.exams.i_total_marks;
        }
    ...
    $rootScope.submit_frm_edit = function(edit_fields)
    {
        if(form_valid('#frmEdit',"help-block")){
            ...
            PaginationService.submit_edit_form_data(GLOBAL.API_USER_URL+"marks/edit", edit_fields, '/marks');
    ...
    $rootScope.data_active_inactive = function(item){
        $timeout(function() {
            PaginationService.change_active_inactive(GLOBAL.API_USER_URL+"marks/change-status", item.id, item.e_status);
           ...
           else
           {
               ...
               bootbox.confirm("Are you sure you want to "+ massage+" selected record?", function(result) {
                if(result == true)
                {
                  ...
                  var responsePromise = $http.post(GLOBAL.API_USER_URL+'marks/bulk-actions',{'id':$rootScope.ids,'action':status});
                   responsePromise.success(function(data, status1, headers, config) {
                        ...
                        if($status =='del')
                        {
                            PaginationService.loading(GLOBAL.API_USER_URL+"marks","i_mark_obtained"); 
    ...
    $rootScope.delete_btn = function(id)
    {
        bootbox.confirm("Are you sure you want to delete this record?", function(result) {
            if(result == true)
            {
                PaginationService.delete_data(GLOBAL.API_USER_URL+"marks/marks-delete",id);
    ...
    $scope.$on('$viewContentLoaded', function() {
        ...
        PaginationService.loading(GLOBAL.API_USER_URL+"marks","i_batch_id");
        $rootScope.get_all_common_list($scope.sectionName); 
...
'use strict';
app.controller('AddMarksController', function($rootScope, $scope, $http, $timeout, PaginationService, GLOBAL, mySharedService, $filter, $state, $stateParams) {
    $rootScope.add_fields = {};
    
    $rootScope.add_fields.marks = [];
    $rootScope.add_fields.marksInput = [];
    
    
    $rootScope.batches_list = {};
    $rootScope.exams_list = {};
    ...
    responsePromise.success(function(data, status1, headers, config) 
    {
        ...
        $rootScope.exams_list = data.exams_list;
    });
    
    $scope.selectStudent = function(student_id,$event){
        ...
        $timeout(function(){
            if($(els).is(':checked')){
                ...
                $rootScope.add_fields.marksInput[student_id] = '';
    ...
    /*$scope.addMarks = function(current, active){
        if(active){
            $scope.marks[current.id] = current.value;    
        } else {
            $scope.marks.splice($scope.marks.indexOf(student_id), 1);
        }
    }*/
    ...
    $scope.select_all_user = function() {
        $timeout(function(){
            ...
            if ($scope.selectedAll) {
                ...
                angular.element('.checker').find('.checked').addClass('checked');
    ...
    $rootScope.submit_frm_add = function(add_fields, button_pressed){
       /* console.log($rootScope.add_fields);
        return false;*/
        var limit_of_marks = true;
        angular.forEach($rootScope.add_fields.marksInput, function (marks) {
            if(marks > $rootScope.add_fields.i_total_marks){
                console.log(marks+"---------------"+$rootScope.add_fields.i_total_marks);
                angular.element('#marks_error_msg').html('Total marks of this exam '+$rootScope.add_fields.i_total_marks+', Please enter a lower value compare to '+$rootScope.add_fields.i_total_marks+'.');
                limit_of_marks = false;      
            }
        });
        if(form_valid('#frmAdd',"help-block") && limit_of_marks){
            $rootScope.add_fields.i_student_ids = $scope.student_id;
            if(button_pressed == 'New'){
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"marks/add", add_fields);
            } else {
                PaginationService.submit_add_form_data(GLOBAL.API_USER_URL+"marks/add", add_fields, '/marks');
            }
    ...
    $scope.getExam = function(id){
        if(id != '' && id !== undefined)
        {
            var activityObj = $filter('filter')($rootScope.exams_list, function (d) {
                return d.id == id;
            })[0];
            $rootScope.add_fields.i_total_marks = activityObj.i_total_marks;   
        } else {
            $rootScope.add_fields.i_total_marks = ''; 
        }
    }
    /* End add form */
